...
//-----------------------------------------------------------------------------------------------
// 体力
const int CObstacle::LIFE = 2000;
// 幅
const float CObstacle::SIZE_WIDTH = 300.0f;
// 高さ
const float CObstacle::SIZE_HEIGHT = 270.0f;
// U座標の最大分割数
const int CObstacle::DIVISION_U = 4;
// V座標の最大分割数
const int CObstacle::DIVISION_V = 1;
// 攻撃前の予備動作間隔
const int CObstacle::RUSH_OPERATION = 60;

//-----------------------------------------------------------------------------------------------
// コンストラクタ
//-----------------------------------------------------------------------------------------------
CObstacle::CObstacle() :m_pattern(PATTERN_ENTRY), m_nCounter(0), m_fAttackRot(0.0f), m_bSizeChange(false),
...
// 当たり判定
//-----------------------------------------------------------------------------------------------
bool CObstacle::Collision(D3DXVECTOR2 posStart)
{
	//サイズ取得
...
// ダメージ処理
//-----------------------------------------------------------------------------------------------
void CObstacle::Damage(int nDamage, CPlayer* pPlayer)
{
	//// ボスの死亡フラグ取得
	//bool bDie = CGame::GetDieBoss();
...
// 行動パターン管理
//-----------------------------------------------------------------------------------------------
bool CObstacle::Pattern(D3DXVECTOR3& pos, D3DXVECTOR2& size, D3DXVECTOR3& move)
{
	////雑魚敵生成情報の取得
...
// 拡縮処理
//-----------------------------------------------------------------------------------------------
void CObstacle::ChangeSize(D3DXVECTOR2 *pSize, const float& fSize)
{
	//if (!m_bSizeChange)
	//{
